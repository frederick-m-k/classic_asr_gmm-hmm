info_mess="This scripts runs an evaluation for a triphone model"
usage_mess="
Usage: Usage:  $0 <tri_model_dir> <data_dir> <which_test_set> <lm_dir>
                <lmwt_range_start> <lmwt_range_end>
                <lmwt_range_step>
        <model_dir>: the path to the directory in which the
                model files are located, e.g. exp/tri, ...
...
./scripts/task31-32Tri.sh $model_dir $data_dir $which_test $lm_dir

./scripts/task34.sh $model_dir $data_dir $which_test $lmwt_start $lmwt_end $lmwt_step

echo "If you want to have an overview of the amount of phonemes in the dataset"
echo "Please use scripts/utils/amount_phonemes.py"
